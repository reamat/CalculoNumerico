...
&\vdots&\\
a_0+a_1(x_n-x_0)+a_2(x_n-x_0)(x_n-x_1)+\cdots + a_n(x_n-x_0)(x_n-x_1)\cdots (x_n-x_{n-1})
&=&y_n\\
\end{eqnarray*}  
\end{tiny}
...
\vdots&\vdots&\vdots&\ddots&\vdots\\
1&(x_n-x_0)&(x_n-x_0)(x_n-x_1) & \cdots& (x_{n}-x_0)(x_n-x_1)\cdots(x_n-x_{n-1})\\
\end{array}
\right]
\left[
\begin{array}{c}
...
\begin{prob} 
Considere o seguinte conjunto de pontos: $$(-2,-47),(0,-3),(1,4)(2,41)$$. Encontre o polinômio interpolador usando os métodos vistos. Trace os pontos no Scilab usando o comando 'plot2d' e trace o gráfico do polinômio usando comandos de plotagem e a estrutura de polinômio.
{\bf Resp:} $5x^3+2x-3$
\end{prob}
...
\begin{eqnarray*}
\int_0^1 P(x)dx&=&\left[f(0)\left(\frac{2}{3}x^3-\frac{3}{2}x^2+x\right)+f\left(\frac{1}{2}\right)\left(-\frac{4}{3}x^3+2x^2\right)+f(1)\left(\frac{2}{3}x^3-\frac{1}{2}x^2\right)\right]_0^1\\
...
&=& \frac{M}{6}  \left[\int_0^{1/2}x\left(x-\frac{1}{2}\right)(x-1)dx-\int_{1/2}^1x\left(x-\frac{1}{2}\right)(x-1)dx\right]\\
...
\begin{prob}Use as mesmas técnicas usadas o resultado do exemplo (\ref{exemp_simpson}) para obter uma aproximação do valor de
$$\int_0^1 f(x)dx$$
através do polinômio interpolador que coincide com $f(x)$ nos pontos $x=0$ e $x=1$.
\end{prob}
{\bf Resp:} $\int_0^1 P(x)dx =\frac{f(0)+f(1)}{2}$, $\frac{1}{12}\max_{x\in[0,1]}|f''(x)|$
...
\begin{prob}Encontre a função do tipo $f(x)=bx+a$ que melhor aproxima os pontos do problema anterior.
\end{prob}

{\bf Resp: $f(x)=-0,3 +2,3x $}
...
\begin{ex} Encontrar  a reta que melhor aproxima o seguinte conjunto de dados:
...
\begin{prob}Encontrar  a parábola $y=ax^2+bx+c$ que melhor aproxima o seguinte conjunto de dados:
\begin{tabular}{|c|c|}
\hline
$x_i$ & $y_i$\\
\hline
...
{\bf Resposta} $y=-0,0407898x^2+ 2,6613293x+ 1,9364598$
...
\begin{prob} Dado o seguinte conjunto de dados
$$
\begin{tabular}{|c|c|}
\hline
$x_i$ & $y_i$\\
...
{\bf Resp:} $a=25,638625$, $b=9,8591874$, $c=4,9751219$ e   $a=31,475524$, $b=65,691531$, $c=-272,84382$, $d=208,23621$.
...
\frac{\partial E_q}{\partial b}&=&2Ax_0(Ae^{x_0b}-y_0)e^{x_0b}+2Ax_1(Ae^{x_1b}-y_1)e^{x_1b}+2x_2A(Ae^{x_2b}-y_2)e^{x_2b}=0
\end{eqnarray*}
...
A Figura \ref{fig:linear_segmentada} é um esboço da função $f(x)$ obtida. Ela foi gerada no Scilab usando os comandos:
\verbatiminput{./cap_aproxfun/codes/interpolacao_linear_segmentada/ex_linear_segmentada.sce}

Veja a Figura \ref{fig:linear_segmentada}.
...
\end{eqnarray*}
\begin{eqnarray*}
b_1&=&\frac{y_{2}-y_1}{h_1}-\frac{h_1}{3}(2c_1+c_{2})=\frac{-1,9-4,5}{3}-\frac{3}{3}(2\cdot 0-0,7)=-2,8333333\\
b_2&=&\frac{y_{3}-y_2}{h_2}-\frac{h_2}{3}(2c_2+c_{3})=\frac{0,5-(-1,9)}{4}-\frac{4}{3}(2\cdot 0,7+0,4)=-0,7333333\\
b_3&=&\frac{y_{4}-y_3}{h_3}-\frac{h_3}{3}(2c_3+c_{4})=\frac{-0,5-0,5}{3}-\frac{3}{3}(2\cdot (-0,4)+0)=0,4666667
\end{eqnarray*}
Portanto,
\begin{small}
\begin{equation*}
f(x)=\left\{\begin{array}{ll}
4,5-2,8333333(x-2)+0,0777778(x-2)^3 &, 2\leq x<5\\
-1,9-0,7333333(x-5)+0,7(x-5)^2-0,0916667(x-5)^3 &, 5\leq x<9\\
0,5+0,4666667(x-9)-0,4(x-9)^2+0,0444444(x-9)^3 &, 9\leq x\leq 12
\end{array}\right..  
\end{equation*}  
\end{small}
...
\subsection{Spline com condições de contorno fixadas}
...
\end{eqnarray*}
\begin{eqnarray*}
b_1&=&\frac{y_{2}-y_1}{h_1}-\frac{h_1}{3}(2c_1+c_{2})=\frac{1-0}{\pi/2}-\frac{\pi/2}{3}(2\cdot (-0,0491874)-0,5956302)=1\\
b_2&=&\frac{y_{3}-y_2}{h_2}-\frac{h_2}{3}(2c_2+c_{3})=\frac{0-1}{\pi/2}-\frac{\pi/2}{3}(2\cdot(-0,5956302) +0)=-0,0128772\\
b_3&=&\frac{y_{4}-y_3}{h_3}-\frac{h_3}{3}(2c_3+c_{4})=\frac{-1-0}{\pi/2}-\frac{\pi/2}{3}(2\cdot 0+0,5956302)=-0,9484910\\
b_4&=&\frac{y_{5}-y_4}{h_4}-\frac{h_4}{3}(2c_4+c_{5})=\frac{0-(-1)}{\pi/2}-\frac{\pi/2}{3}(2\cdot 0,5956302+0,0491874)=-0,0128772
\end{eqnarray*}
Portanto,
\begin{equation*}
f(x)=\left\{\begin{array}{ll}
x-0,0491874x^2-0,1159588x^3&, 0\leq x<\frac{\pi}{2}\\
1+-0,0128772(x-\frac{\pi}{2})-0,5956302(x-\frac{\pi}{2})^2+0,1263967(x-\frac{\pi}{2})^3&, \frac{\pi}{2}\leq x<\pi\\
-0,9484910(x-\pi)+0,1263967(x-\pi)^3&, \pi\leq x<\frac{3\pi}{2}\\
-1-0,0128772(x-\frac{3\pi}{2})+0,5956302(x-\frac{3\pi}{2})^2-0,1159588(x-\frac{3\pi}{2})^3&, \frac{3\pi}{2}\leq x\leq2\pi
\end{array}\right..  
\end{equation*}

No Scilab, podemos resolver este problema fazendo:
\verbatiminput{./rotinas/Interpolacao_e_ajuste/ex_spline_fixado.sce}
...
\begin{equation*}
  s(x) = \left\{\begin{array}{ll}
      ...
      s_2(x) = a_2 + b_2(x-x_2) + c_2(x-x_2)^2 + d_2(x-x_2)^3 &, x_2 \leq x < x_3\\
      \qquad \vdots & \qquad\vdots \\
      s_{n-1} = a_{n-1} + b_{n-1}(x-x_{n-1}) + c_{n-1}(x-x_{n-1})^2 + d_{n-1}(x-x_{n-1})^3 &, x_{n-1} \leq x \leq x_n \end{array}\right.
\end{equation*}

Definindo-se $h_j = x_{j+1} - x_j$, os coeficientes $c_j$, $j=1,2,\dotsc,n$, são solução do sistema linear $Ac = B$, onde:
\begin{small}
  \begin{equation*}
