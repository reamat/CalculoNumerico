...
\section{Método de Adams-Bashforth}\index{Método!de Adams-Bashforth}\index{Método!de passo múltiplo}
...
\begin{equation}\label{pvi:betaj_s}
...
\begin{ex} Calcule os coeficientes e de Adams-Bashforth para $s=2$ e, depois, construa o processo iterativo.
 \begin{eqnarray*}
  \beta_0&=&\int_0^1 \frac{\tau+2-1-1}{0-1}d\tau=-\frac{1}{2}\\
  \beta_1&=&\int_0^1 \frac{\tau+2-0-1}{1-0}d\tau=\frac{3}{2}
 \end{eqnarray*}
...
\begin{ex} Calcule os coeficientes e de Adams-Bashforth para $s=3$ e, depois, construa o processo iterativo.
 \begin{eqnarray*}
  \beta_0&=&\int_0^1 \frac{(\tau+3-1-1)}{0-1}\cdot \frac{(\tau+3-2-1)}{0-2} d\tau=\frac{5}{12}\\
  \beta_1&=&\int_0^1 \frac{(\tau+3-0-1)}{1-0}\cdot \frac{(\tau+3-2-1)}{1-2} d\tau=-\frac{4}{3}\\
  \beta_2&=&\int_0^1 \frac{(\tau+3-0-1)}{2-0}\cdot \frac{(\tau+3-1-1)}{2-1} d\tau=\frac{23}{12}
 \end{eqnarray*}
 O processo iterativo é dado por:
 \begin{equation*}
...
\subsection*{Exercícios resolvidos}
\begin{exeresol} Resolva numericamente o problema de valor inicial dado por:
\begin{eqnarray*}
 y'(t)&=& \sqrt{1+y(t)}\\
 y(0)&=& 0
\end{eqnarray*}
...
\begin{equation*}
  y^{(n+2)}=y^{(n)}+\frac{h}{2}\left[3f\left(t^{(n+1)},u(t^{(n+1)})\right)-f\left(t^{(n)},u(t^{(n)})\right)\right],~n=1,2,\ldots
 \end{equation*}
...
\section{Método de Adams-Moulton}\index{Método!de Adams-Moulton}
...
\begin{eqnarray}\label{pvi:mpm2}
  u^{(n+s)}  &=& u^{(n+s-1)}  + \int_{t^{(n+s-1)}}^{t^{(n+s)}} f(t,u(t)) dt
\end{eqnarray}
Agora o integrando em \eqref{pvi:mpm2} é aproximado pelo polinômio que interpola $f(t^{(k)},u^{(k)})$ para $k = n, n+1, n+2, \ldots, n+s$, isto é:
...
onde 
\begin{equation}\label{pvi:betaj_m}
...
\subitem Expressão forma-fechada: Expressões envolvendo apenas um número finito de operações aritméticas elementares, expoentes reais, logaritmos, exponencias, funções trigonométricas e funções trigonométricas inversas. Ex: $u(t)=\log(1+t^{\pi})$, $u(t)=e^{-t}\sin(t)$ ou $u(t)=\tan^{-1}(t+1)$.
...
\item Expressão matemática semi-analítica: Expressão matemática envolvendo, além das operações e funções acima, outros processos de limite, como derivação e integração. Ex: $u(t)=\int_{0}^1 \log|t-s|x^t dx$.  
...
\begin{ex}[Expressão matemática semi-analítica]
