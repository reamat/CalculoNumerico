...
\begin{ex}\label{ex:duas_sequencias}
Considere a equação:
\begin{equation*}
  xe^x = 10.
\end{equation*}
Esta é equivalente às seguintes equações (problemas de ponto fixo):
\begin{equation*}
...
A iteração do ponto fixo aplicada a estas duas equações nos fornece as seguintes sequências:
\begin{enumerate}
 \item [a)] $\displaystyle x^{(n+1)}=\ln \left(\frac{10}{x^{(n)}}\right)$
 \item [b)] $\displaystyle x^{(n+1)}=10 e^{-x^{(n)}} $
\end{enumerate}

Tomemos $x^{(1)}=1$ como aproximação inicial para a solução deste problema, iterando a primeira sequência, obtemos:
\begin{align*}
x^{(1)} &=1\\
...
Iterando a segunda sequência, obtemos:
\begin{align*}
x^{(1)}&=1\\
x^{(2)}&=10e^{-1}= 3,6787944   \\
...
\begin{ex}
Considere o problema de encontrar a solução da equação:
\begin{equation*}
  \cos(x)=x
\end{equation*}
Isto é, queremos computar o ponto fixo da $g(x) := \cos(x)$. Mostraremos que o teorema do ponto fixo se aplica a esta função com $[a, b]=[1/2, 1]$. Precisamos mostrar que:
\begin{enumerate}
\item[a)] $g\left([1/2,1]\right) \subseteq [1/2,1]$;
\item[b)] $|g'(x)|<\beta, \quad \beta<1,\quad \forall x\in [1/2,1]$.
\end{enumerate}
