...
\begin{figure}
  ...
  \caption{Exemplo de interpolação de dois pontos por uma reta, veja o Exemplo~\ref{ex:intro_interpolacao}.}
  \label{fig:ex_intro}
\end{figure}
...
\begin{ex}\label{ex:intro_interpolacao}
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
o que nos fornece $a = 0$ e $b = 1$. Então, a função interpoladora é $f(x) = x$. Os pontos e a reta interpolada estão esboçados na Figura~\ref{fig:ex_intro}.
\end{ex}
...
\begin{ex} Encontre o polinômio interpolador do conjunto de pontos $\{(0, 1)$, $(1, 2)$, $(2, 4)$, $(3, 8)\}$
  \begin{figure}
    \centering
    \includegraphics{./cap_interp/pics/ex_interpolacao_polinomial/ex_interpolacao_polinomial}
    \caption{Polinômio interpolador do conjunto de pontos $\{(0, 1)$, $(1, 2)$, $(2, 4)$, $(3, 8)\}$. Veja o Exemplo~\ref{ex:interpolacao_polinomial}.}
    \label{fig:ex_interpolacao_polinomial}
  \end{figure}
.
\end{ex}\label{ex:interpolacao_polinomial}
\begin{sol}
