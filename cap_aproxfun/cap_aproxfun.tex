...
\chapter{Interpolação e ajuste de curvas}\index{aproximação!de funções}

Neste capítulo, discutimos sobre problemas de \emph{interpolação}\index{interpolação} e de \emph{ajuste de curvas}\index{ajuste de curvas} pelo \emph{método dos mínimos quadrados}\index{método dos mínimos quadrados}. Mais precisamente, seja dado um conjunto de $n$ pontos $\left\{(x_i, y_i)\in \mathbb{R}^2\right\}_{i=1}^n$ e uma família de funções $\mathcal{F} = \{f:\mathbb{R}\to\mathbb{R}; y = f(x)\}$. No problema de interpolação, queremos encontrar uma função $f\in\mathbb{F}$ tal que:
\begin{equation*}
...
\begin{figure}
  \centering
  \includegraphics[scale=0.9]{./cap_aproxfun/pics/ex_intro_interpolacao/ex_intro_interpolacao}
~
  \includegraphics[scale=0.9]{./cap_aproxfun/pics/ex_intro_ajuste/ex_intro_ajuste}
  \caption{Exemplo de problemas de interpolação e ajuste. Esquerda: problema de interpolação, veja o Exemplo~\ref{ex:intro_interpolacao}. Direita: problema de ajuste, veja o Exemplo~\ref{ex:intro_ajuste}.}
  \label{fig:ex_intro}
\end{figure}
...
\begin{ex}\label{ex:intro_interpolacao}
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
o que nos fornece $a = 0$ e $b = 1$. Então, a função interpoladora é $f(x) = x$. Veja o gráfico à direita na Figura~\ref{fig:ex_intro}.
\end{ex}
...
\begin{ex}\label{ex:intro_ajuste}
  Dados o conjunto de os pontos $\{(1, 1,2), (1,5, 1,3), (2, 2,3)\}$ e a família de retas $f(x) = a + bx$, podemos mostrar que $f(x) = -0,05 + 1,1x$ é a reta que melhor aproxima os pontos dados no sentido de mínimos quadrados.  Veja o gráfico à direita na Figura~\ref{fig:ex_intro}. Para mais detalhes em como obter a função $f(x)$, veja a Seção~\ref{sec:ajuste_de_curvas} para maiores detalhes. 
\end{ex}
...
% Seja  um conjunto de pares ordenados tais que $x_i \ne x_j$ se $i\ne j$, encontrar  uma função $f \in \mathcal{F}$ (uma dada família de funções) tal que
% $$f(x_i)=y_i, \;\;\; i=1,...,n$$

% \begin{ex} Encontrar a função $f(x)$ da forma $f(x)=a e^{bx}$ onde $a$ e $b$ são constantes tal que $f(1)=1$ e $f(2)=5$. Este problema equivale a resolver o seguinte sistema de equações:
% \begin{eqnarray*}
% ae^b&=&1\\
% ae^{2b}&=&5
% \end{eqnarray*}
...
\section{Ajuste de curvas}\label{sec:ajuste_de_curvas}\index{método!dos mínimos quadrados}\index{ajuste de curvas}
...
Por exemplo, dado um conjunto de $15$ pontos como na figura \ref{minquad}, encontrar a reta que melhor se ajusta aos pontos.
\begin{figure}[htp]
\begin{center}
