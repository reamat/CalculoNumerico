...
\begin{teo}
 ...
 Além disso, a sequência
  $$x^{(n+1)}=g(x^{(n)})$$
é convergente para qualquer $x_1\in[a,b]$ e $$\lim_{n\to\infty}x^{(n)}=p.$$
\end{teo}
...
e
$$f(b)=b-g(b)\geq b-b=0$$
Se $f(a)=a$ ou $f(b)=b$, então o ponto fixo existe. Caso contrário, as desigualdade são estritas e a função muda de sinal no intervalo. Como a função é contínua, pelo teorema do valor intermediário, existe um ponto $x^*$ no intervalo $(a,b)$ tal que $f(x^*)=0$, ou seja, $x^*-g(x^*)=0$. Observe que $x^*$ é um ponto fixo de $g$, pois $g(x^*)=x^*$. 
