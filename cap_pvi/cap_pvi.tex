...
Para simplificar a notação, denote $u_n := u(t_n)$ como a solução aproximada em $t_n$ e temos
\begin{eqnarray}
  u_2-u_1       &=& \int _{t_1}^{t_2} f(t,u(t)) \; dt  \\
  u_2           &=& u_1 +  \int _{t_1}^{t_2} f(t,u(t)) \; dt
\end{eqnarray}

Aproximando $f$ por uma função constante $f(t,u(t)) \approx  f(t_1,u_1) \equiv  f_1$  em $(t_1,t_2)$,
\begin{eqnarray}
  u_2 &\approx&  u_1 +   f(t_1,u_1) \int _{t_1}^{t_2}  \; dt \\
  u_2 &\approx&  u_1 +   f(t_1,u_1) (t_2-t_1) \\
  u_2 &\approx&  u_1 + h f(t_1,u_1)
\end{eqnarray}
...
$$u'(t)=\frac{u(t+h)-u(t)}{h}+O(h),~~ h>0.$$
Substituindo na EDO temos
\begin{eqnarray}
\frac{u(t+h)-u(t)}{h}&=&f(t,u(t))+O(h)\\
u(t+h)&=&u(t)+hf(t,u(t))+O(h^2).{\label{erro_local}}
\end{eqnarray}

Sendo $u_n:=u(t_n)$ a aproximação de $u$ em $t_n$ produzida pelo método de Euler, obtemos
...
\end{eqnarray*}
\end{ex}
cuja solução é $u(t)=e^{2t}$. O método de Euler aplicado a este problema produz o seguinte esquema:
\begin{eqnarray*}
...
No exemplo \ref{sys_edo}, mostramos como o m de Euler pode ser facilmente estendido para problemas envolvendo sistemas de equações diferenciais.\index{sistemas!de equações diferenciais}.
...
\begin{eqnarray*}
\left[\begin{array}{c}x_{k+1}\\y_{k+1}\end{array}\right]=\left[\begin{array}{c}x_k\\y_k\end{array}\right]+h\left[\begin{array}{c}-y_k\\x_k\end{array}\right].
\end{eqnarray*}
...
\end{ex}
A fim de transformar a equação diferencial dada em um sistema de equações de primeira ordem, introduzimos a substituição $w=u'$, de forma que obteremos o sistema:
\begin{eqnarray*}
u'&=&w\\
w'&=&-w-u+\cos(t)\\
u(0)&=&1\\
...
\begin{eqnarray*}
u_{k+1}&=&u_k+hw_k,\\
w_{k+1}&=&w_k-hw_k-hu_k+h\cos(t_k),\\
u_1&=&1,\\
w_1&=&0.
\end{eqnarray*}
...
Substituindo a equação diferencial $u'=f(t,u)$ na expansão em série de Taylor
...
obtemos
\begin{eqnarray}
   u_{n+1}-u(t_{n+1}) =&u_n - u(t_n)  +\\
                       &+ h(f(t_n,u(t_n)+e_n)- f(t_n,u(t_n)))  +\frac{h^2}{2}u''_n+\mathcal O(h^3)
\end{eqnarray}
...
\begin{eqnarray}
   |e_{n+1}|      &\leq  |e_n| + h|f(t_n,u(t_n)+e_n)- f(t_n,u(t_n))|+\frac{h^2}{2}|u''_n|+\mathcal O(h^3)\\
                  &\leq  |e_n| + hL |u(t_n)+e_n- u(t_n)|+\frac{h^2}{2}|u''_n|+\mathcal O(h^3)\\
                  &\leq  |e_n| + hL |e_n|+\frac{h^2}{2}|u''_n|+\mathcal O(h^3)\\
                  &\leq  (1+ hL) |e_n|+\frac{h^2}{2}|u''_n|+\mathcal O(h^3)
\end{eqnarray}
...
\begin{eqnarray}
 \lim_{h\rightarrow 0} |e_{n+1}|  &= \lim_{h\rightarrow 0} \mathcal O(h) = 0
\end{eqnarray}
...
\begin{eqnarray*}
u_{k+1}&=&u_k+hf(u_k,t_k)\\
u_1&=&u_0~~ \hbox{(condição inicial)}
\end{eqnarray*}

A ideia do método de Euler melhorado é substituir a declividade  $f(u_k,t_k)$ pela média aritmética entre $f(u_k,t_k)$ e $f(u_{k+1},t_{k+1})$, isto é, as declividades avaliadas no início e no fim do intervalo $[t_k,t_{k+1}]$.

No entanto, não dispomos do valor de $u_{k+1}$ antes de executar o passo. Assim aproximamos esta grandeza pelo valor produzido pelo método de Euler original:
$$\tilde{u}_{k+1}=u_k+hf(u_k,t_k).$$
...
\begin{eqnarray*}
\tilde{u}_{k+1}&=&u_k+hf(u_k,t_k)\\
u_{k+1}&=&u_k+\frac{h}{2}\left[f(u_k,t_k)+f(\tilde{u}_{k+1},t_{k+1})\right]\\
u_1&=&u_0~~ \hbox{(condição inicial)}
\end{eqnarray*}
...
\begin{eqnarray*}
k_1&=&hf(u_k,t_k),\\
k_2&=&hf(u_k+k_1,t_{k+1}),\\
u_{k+1}&=&u_k+\frac{k_1+k_2}{2},\\
u_1&=&u_0~~ \hbox{(condição inicial)}.
\end{eqnarray*}
...
\begin{eqnarray}{\label{euler_precisao}}
 u(t_N)&=&=u_{N}+\frac{t-t_0}{h}O(h^2)=u_{N}+O(h),
\end{eqnarray}
