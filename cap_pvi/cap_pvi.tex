...
\end{eqnarray}
No método de Adams, o integrando em \eqref{pvi:mpm1} é aproximado pelo polinômio que interpola $f(t^{(k)},u^{(k)})$ para $k = n, n+1, n+2, \ldots, n+s-1$, isto é:
\begin{eqnarray}\label{pvi:mpm1}
...
\begin{exer} Calcule os coeficientes e de Adams para $s=2$ e, depois, construa o processo iterativo.
 \begin{eqnarray*}
 ...
 \begin{equation*}
  y_{n+2}=y_n+\frac{h}{2}\left[3f\left(t^{(n+1)},u(t^{(n+1)})\right)-f\left(t^{(n)},u(t^{(n)})\right)\right]
 \end{equation*}
