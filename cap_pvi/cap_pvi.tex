...
\section{O método de Euler melhorado}\index{Método de Euler melhorado} 
...
Portanto, o método recursivo de Euler modificado assume a seguinte forma:
\begin{eqnarray*}
k_1&=&hf(u^{(k)},t^{(k)}),\\
k_2&=&hf(u^{(k)}+k_1,t^{(k+1)}),\\
u^{(k+1)}&=&u^{(k)}+\frac{k_1+k_2}{2},\\
