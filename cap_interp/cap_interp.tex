...
h_jy'_{j-1}+2(h_{j-1}+h_j)y'_j+h_{j-1}y'_{j+1}=3\left(\dfrac{h_j}{h_{j-1}}(y_j-y_{j-1})+\dfrac{h_{j-1}}{h_j}(y_{j+1}-y_j)\right),
\end{equation}
...
\end{array}\right]\qquad \hbox{e}\qquad
z = 3\left[\begin{array}{c}
	\frac{h_2}{h_1}(y_2-y_1)+\frac{h_1}{h_2}(y_3-y_2)-h_2y'_1\\
	\frac{h_3}{h_2}(y_3-y_2)+\frac{h_2}{h_3}(y_4-y_3)\\
	\vdots\\
	\frac{h_{n-2}}{h_{n-3}}(y_{n-2}-y_{n-3})+\frac{h_{n-3}}{h_{n-3}}(y_{n-1}-y_{n-2})\\
	\frac{h_{n-1}}{h_{n-2}}(y_{n-1}-y_{n-2})+\frac{h_{n-2}}{h_{n-1}}(y_n-y_{n-1})-h_{n-2}y'_n\\
\end{array}\right]
\end{small}
\end{equation*}
...
\begin{equation*}
	\begin{small}
		...
		z = \left[\begin{array}{c}
			...
			3\left(\frac{h_2}{h_1}(y_2-y_1)+\frac{h_1}{h_2}(y_3-y_2)\right)\\
			\vdots\\
			3\left(\frac{h_{n-1}}{h_{n-2}}(y_{n-1}-y_{n-2})+\frac{h_{n-2}}{h_{n-1}}(y_n-y_{n-1})\right)\\
...
O termo \textit{not-a-knot} (não nó) relaciona-se à nomenclatura dos splines. O termo "nó" é utilizado para os pontos interpolados. Neles, a derivada terceira da função spline é descontínua, portanto quando impomos a continuidade dessa derivada em $x_2$ e $x_{n-1}$ é como se esses pontos deixassem de ser nós.

\subsection{Spline periódico}\index{spline!periodico}
Se o conjunto de $n$ pontos da interpolação $\mathcal{I}$ for tal que $y_1=y_n$, então é possível construir o spline periódico, definido com um spline cúbico que satisfaz as equações adicionais
\begin{equation*}
