...
isto é:
$$u-\frac{h}{2}\left(u^3+t^{(n+1)}\right)=a,$$
com $a=y^{(n)}+\frac{h}{2}f\left(t^{(n)},u(t^{(n)})\right)$
Os valores obtidos são: $0,51988523$ e $0,51906493$ quando o método é inicializadom com Euler melhorado.
\ifispython
A seguinte rotina implementa a recursão:
\begin{verbatim}
 def f(t,u):
...
def resolve(u,t,a,h,beta): #resolve equacao nao-linear por metodo de Newton
	...
	while (ctrl>0):
		cont=cont+1
		residuo=u-beta*h*(u**3+t)-a
		derivada=1-beta*h*3/2*u**2
