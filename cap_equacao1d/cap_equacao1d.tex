...
Queremos calcular $x^{(1)}$ em função de $x^{(0)}$. A ideia é calcular a equação da reta tangente à função $y=f(x)$ no ponto de abscissa $x^{(0)}$ e definir $x^{(1)}$ como a abscissa do ponto em que esta reta cruza o eixo x.  A inclinação da reta tangente é dada por (a variação em $y$ sobre a variação em $x$):
\begin{equation*}
   f'(x^{(0)}) = \frac{ f(x^{(0)})-0}{x^{(0)}-x^{(1)} }
\end{equation*}
Como desejamos que a abscissa do ponto em que esta reta cruza o eixo x seja $x^{(1)}$, obtemos:
\begin{equation*}
