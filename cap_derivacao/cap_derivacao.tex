...
Uma diferença finita é simplesmente uma expressão da forma $f(x+b)-f(x+a)$, que ao ser dividida por $(b-a)$ chama-se um quociente de diferenças. 

A técnica de \emph{diferenças finitas}\index{diferenças finitas} consiste em aproximar a derivada de uma função via fórmulas discretas que requerem apenas um conjunto finito de pares ordenados $\left\{\left(x_i, y_i\right)\right\}_{i=1}^n$, onde geralmente temos que $y_i=f(x_i)$.

Essas fórmulas podem ser obtidas de várias maneiras. Começamos com a fórmula mais simples que pode ser obtida do Cálculo diferencial.

Seja $f$ uma função diferenciável, a derivada de $f$ no ponto $x_0$ é, por definição,
\begin{equation*}
...
\left(
  ...
  \end{array}
\right)
\left(\begin{array}{c}  c_1 \\ c_2\\ c_3  \\ \vdots   \\ c_n     \end{array}\right)
=
\left(\begin{array}{c}  0  \\ 1 \\ 2x^* \\ \vdots  \\ (n-1)(x^*)^{n-2}  \end{array}\right)
\end{eqnarray}
...
\begin{eqnarray}
  f'|_{x=x_1}& \approx &  -\frac{1}{2h}f_1+\frac{1}{2h}f_3\\
             & \approx &\frac{f_3-f_1}{2h}
\end{eqnarray}
\end{sol}
