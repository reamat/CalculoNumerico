...
Vamos, agora, discurtir sobre a \emph{convergência} do método da bisseção. O próximo Teorema~\ref{teo:convergencia_bissecao} nos garante a convergência do método da bisseção.
...
\begin{equation*}
  \frac{b^{(n)} - a^{(n)}} = \frac{b^{(n-1)}-a^{(n-1)}}{2}.
\end{equation*}
Desta forma:
\begin{equation*}
...
Observamos que a hipótese de que $f(x)$ tenha um único zero no intervalo não é necessária. Se a função tiver mais de um zero no intervalo inicial, as iterações irão convergir para um dos zeros. Veja o Exercício~\label{exer:multiplos_zeros}.

\begin{obs}
  O Teorema~\ref{teo:convergencia_bissecao} nos fornece uma estivamtiva para a convergência do método da bisseção. Aproximadamente, temos:
  \begin{equation*}
...
\begin{ex}
  ...
  \item[b)] É possível usar o método da bisseção para a raíz $x=2$? Justifique sua resposta.
  \end{itemize}
\end{ex}
