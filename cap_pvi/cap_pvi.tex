...
\begin{eqnarray*}
 ...
 &=&y(t_1)+hf(y_1+ch^2,t_1)+ch^2\\
 &=&y(t_1)+hf(y_1,t_1) +ch^2\\
 &=&y_1+ch^2+hf(y_1,t_1) +ch^2= y_2+2ch^2.
\end{eqnarray*}
onde usamos o primeiro termo da série de Taylor $hf(y_1+ch^2,t_1)=hf(y_1,t_1)+O(h^3)$ na passagem da segunda para terceira linha. Repetindo sucessivamente o passo anterior, obtemos uma expressão geral para o valor exato $y(t_N)$ em termos do valor aproximado $y_N$:
\begin{eqnarray*}
 y(t_N)=y_N+Nch^2.
\end{eqnarray*}
...
Para obter o erro acumulado em um ponto $t_N$ usamos a expressão (\ref{es_Euler_melhorado}) e repetimos alguns passos feitos na seção anterior:
\begin{eqnarray*}
...
&=&y(t_1)+\frac{h}{2}f(y_1+ch^3,t_1)+\frac{h}{2}f(y_1+ch^3+hf(y_1+ch^3,t_1),t_1+h)+ch^3\\
&=&y(t_1)+\frac{h}{2}f(y_1,t_1)+\frac{h}{2}f(y_1+hf(y_1,t_1),t_1+h)+ch^3\\
&=&y_2+ch^3+ch^3=y_2+2ch^3,
\end{eqnarray*}
onde usamos o primeiro termo da série de Taylor $hf(y_1+ch^3,t_1)=hf(y_1,t_1)+O(h^4)$ na passagem da segunda para terceira linha. Assim obtemos um expressão geral para o valor exato $y(t_N)$ em termos do valor aproximado $y_N$:
\begin{eqnarray*}
 y(t_N)=y_{N}+Nch^3.
\end{eqnarray*}
Como $N=(t_f-t_0)/h$, temos
