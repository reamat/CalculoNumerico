...
Neste capítulo, discutimos sobre problemas de \emph{interpolação}\index{interpolação}. Mais precisamente, dado um conjunto com $n$ pontos $\left\{(x_i, y_i)\in \mathbb{R}^2\right\}_{i=1}^n$ e uma família de funções $\mathcal{F} = \{f:\mathbb{R}\to\mathbb{R}; y = f(x)\}$, o problema de interpolação consiste em encontrar uma função $f\in\mathbb{F}$ tal que:
\begin{equation*}
...
\begin{ex}\label{ex:intro_interpolacao}
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
o que nos fornece $a = 0$ e $b = 1$. Então, a função interpoladora é $f(x) = x$. Os pontos e a reta interpolação estão esboçados na Figura~\ref{fig:ex_intro}.
\end{ex}
