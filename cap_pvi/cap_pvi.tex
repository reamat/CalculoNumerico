...
No exemplo a seguir, apresentamos um problema envolvendo uma equação não-autônoma\index{equção diferencial!não autônoma}, isto é, quando a função $f(y,t)$ depende explicitamente do tempo.
...
y(0)&=&y_0.
\end{eqnarray*}
Nessa seção vamos definir a precisão de um método numérico pela ordem do erro acumulado ao calcular o valor da função em um ponto $t_N$ em função do espaçamento da malha $h$. 

\subsection{Ordem de precisão do Método de Euler}
Primeiro lembramos da expressão (\ref{erro_local}) que origina o método de Euler:
\begin{eqnarray}{\label{es_Euler}}
y(t+h)&=&y(t)+hf(y(t),t)+O(h^2).
\end{eqnarray}
Assim, se $t=0$, temos $y(h)=y(0)+hf(y_0,t)+O(h^2)= y^{(1)}+O(h^2)$ e concluimos que o erro entre a solução exata $y(h)$ e sua aproximação pelo Método de Euler é da ordem de $h^2$. No entanto, se fixarmos um instante de tempo $t=t_N$, o erro entre a solução exata $y(t_N)$ e sua aproximação $y_N$ envolve o acúmulo de $N$ passos, cada um com erro da ordem de $h^2$. Como o número de passos é inversamente proporcional ao tamanho de passo $h$, o erro total é da ordem de $h$. Observamos que
\begin{eqnarray*}
...
\begin{eqnarray}{\label{euler_precisao}}
 y(t_N)&=&=y_{N}+\frac{t-t_0}{h}ch^2=y_{N}+O(h),
\end{eqnarray}
onde $c_2=(t-t_0)c$, ou seja, o erro entre o valor exato e o aproximado é de ordem $h$. Portanto, dizemos que o método de Euler tem ordem de precisão 1 (ou simplesmente ordem 1). O fenômeno que vimos aqui acontece para todos os métodos numéricos explícitos desde que a função $f$ seja analítica, por isso, vale a pena introduzir a definição abaixo:

\begin{defn}{\label{definicao_precisao_PVI}}
Se $y(t_n)$ pode ser aproximado por uma expressão \\${\bf Y}(f,h,y(t_n),y(t_{n-1}),\cdots,y_0)$ com erro da ordem de $p+1$, ou seja,
\begin{equation}{\label{erro_local_1}}
y(t_{n+1})={\bf Y}(f,h,y(t_n),y(t_{n-1}),\cdots,y_0)+O(h^{p+1})
\end{equation}
para cada função analítica $f$, dizemos que o método tem erro de truncamento da ordem de $O(h^{p+1})$ e {\bf ordem de precisão $p$}\index{método!de Euler!ordem de precisão}. 
\end{defn}
No caso do método de Euler, vale a expressão (\ref{es_Euler}) e, portanto, o método tem erro de truncamento de ordem $O(h^2)$ e ordem de precisão 1. Nos métodos que vamos trabalhar a seguir, ficaremos satisfeitos obtendo o erro de truncamento e usaremos a definição \ref{definicao_precisao_PVI} para concluir que a ordem de precisão é uma unidade menor. 

\subsection{Ordem de precisão do Método de Euler Melhorado}
Para obter a do erro de precisão do método de Euler Melhorado vamos calcular o erro de truncamento do método, ou seja, precisamos demonstrar que:
...
\end{eqnarray*}
Portanto, a expressão (\ref{es_Euler_melhorado}) é válida. Logo, a definição (\ref{definicao_precisao_PVI}) nos faz concluir que o método de Euler Melhorado tem ordem de precisão 2 (ou simplesmente ordem 2).
