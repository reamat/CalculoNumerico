...
\end{equation}
Isto é equivalente a aproximar a área entre o gráfico de $f(x)$ e o eixo $x$ restrito ao intervalo $[x_i, x_{i+1}]$ pelo retângulo de base $h_i$ e altura $f(x_i^*)$ (veja Figura~\ref{fig:}). Consequentemente, de \eqref{eq:integral_particionada} temos
\begin{equation}
  \begin{split}
    I &= \int_{a}^{b}f(x)\,dx = \sum_{i=1}^{n-1}\int_{x_i}^{x_{i+1}}f(x)\,dx\\
    &\approx \sum_{i=1}^{n-1} f(x_i^*)h_i.
  \end{split}
\end{equation}
...
% \begin{ex}
% A Figura~\ref{fig:int_101} mostra um exemplo quando $f(x)=x^2+1$, $0\leq x\leq 2$. Temos a aproximação por um retângulo com base $h_1=2$, depois com dois retângulos de base $h_2=1$ e, finalmente com quatro retângulos de bases $h_3=0,5$.
% \begin{figure}
%   \centering
...
Aqui, utilizamos $x_1:=a$,  $x_2:=b$, $h=x_2-x_1$ e a notação $f_i=f(x_i)$, obtemos através da interpolação de Lagrange o polinômio
\begin{eqnarray}
...
\begin{equation}  I_{Simpson}= \frac{1}{3} I_{Trap}+ \frac{2}{3}I_{PM} \end{equation}
