...
\begin{align*}
  ...
  &= \frac{1}{m}|x^{(n)} - x^{(n-1)}|\left|\frac{f(x^{(n-1)}) - f(x^{(n)})}{x^{(n-1)} - x^{(n)}} - \frac{f(x^{(n-1)}) - f(x^{(n-2)})}{x^{(n-1)} - x^{(n-2)}}\right|.
\end{align*}
Observamos que o último termo pode ser estimado como feito acima para o termo análogo na Inequação~\ref{eq:secantes-est0}. Com isso, obtemos a estimativa desejada:
\begin{equation*}
