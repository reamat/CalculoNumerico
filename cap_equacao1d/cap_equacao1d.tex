...
O experimento numérico sugere que o processo $b$ não é convergente e que o processo $a$ converge para $1,745528$.
...
&=&(x^{(N+1)}-x^{(N)}) \frac{1}{1-\phi'(x^*)}, \qquad \left|\phi'(x^*)\right|<1
\end{eqnarray*}
...
\begin{itemize}
\item Quando $\phi'(x^*)<0$, o esquema é alternante, isto é, o sinal do erro se altera a cada passo.  O erro $\epsilon_N$ pode ser estimado diretamente da diferença $\Delta_N$, pois o denominador $1-\phi'(x^*)>1$.
\item Quando $0<\phi'(x^*)<1$, o esquema é monótono e $\frac{1}{1-\phi'(x^*)}>1$, pelo que o erro $\epsilon_N$ é maior que a diferença $\Delta_N$. A relação será tão mais importante quando mais próximo da unidade for $\phi'(x^*)$, ou seja, quando mais lenta for a convergência. Para estimar o erro em função da diferença $\Delta_N$, observamos que  $\phi'(x^*)\approx \frac{x^{(n+1)}-x^{(n)}}{x^{(n)}-x^{(n-1)}}$ e 
...
Queremos calcular $x^{(1)}$ em função de $x^{(0)}$. A ideia é calcular a equação da reta tangente à função $y=f(x)$ no ponto de abscissa $x^{(0)}$ e definir $x^{(1)}$ como a abscissa do ponto em que esta reta cruza o eixo x.

A reta que passa por $(x^{(0)}, f(x^{(0)})$ e é tangente à curva em $x^{(0)}$ tem inclinação $m=f'(x^{(0)})$, portanto é dada pela equação
$$
y-f(x^{(0)})=f'(x^{(0)})(x-x^{(0)}).
$$
Como desejamos que a abscissa do ponto em que esta reta cruza o eixo x, temos:
$$
0-f(x^{(0)})=f'(x^{(0)})(x^{(1)}-x^{(0)}).
$$
Portanto,
$$x^{(1)}=x^{(0)}-\frac{f(x^{(0)})}{f'(x^{(0)})}$$
que é a iteração do Método de Newton. Repetimos o processo para calcular $x^{(2)},\ x^{(3)},...$. De modo geral, temos:
$$x^{(n+1)}=x^{(n)}-\frac{f(x^{(n)})}{f'(x^{(n)})}.$$
