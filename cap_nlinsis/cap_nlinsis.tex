...
\begin{exer}
\Question Encontre o gradiente da função $$f(x,y)=x^2y+\cos(xy)-4$$
\Question Encontre a matriz jacobiana associada à função
...
\begin{exer}[title=Entenda casos particulares]
\Question Considere a função $L(x)=Ax-b$, onde $A$ é uma matriz $n\times n$ inversível e $b$ um vetor coluna em $\mathbb{R}^n$. O que acontece quando aplicamos o método de Newton para encontrar as raízes de $L(x)$?
\Question Mostre que o método de Newton-Raphson aplicado a uma função diferenciável do tipo $f:\mathbb{R}\to\mathbb{R}$ se reduz ao método de Newton estudado na primeira área.
\end{exer}
...
\begin{figure}
        \centering
	    \includegraphics{cap_nlinsis/pics/curva_Q23}
...
% 
% 
% \begin{exer}{\it (estática)} Considere o sistema mecânico constituído de dois segmentos de mesmo comprimento $L$ presos entre si e a uma parede por articulações conforme a figura
% 
% \includegraphics[width=8cm,angle=0]{dois_segmentos}
% 
...
% 
% 
% \begin{exer}{\it (estática - problemas de três variáveis)} Considere, agora, o sistema mecânico semelhante ao do problema anterior, porém constituído de três segmentos de mesmo comprimento $L$ presos entre si e a uma parede por articulações.
% 
...
% \end{eqnarray*}
% Considere $P=10N$, $L=1m$ e calcule os ângulos $\theta_1$, $\theta_2$ e $\theta_3$ quando:
% \begin{itemize}
% \item[a)] $k=1000 Nm/rad$
% \item[b)] $k=100 Nm/rad$
% \item[c)] $k=10 Nm/rad$
...
% \end{resp}
% 
% \begin{exer}  Encontre os pontos de intersecção das curvas descritas (veja figura) por\end{exer}
...
% \end{eqnarray*}
% \end{minipage}
% %\begin{minipage}[r]{0.7\linewidth} \includegraphics[width=0.7\textwidth]{inter_curvas}\end{minipage}\\
% seguindo os seguintes passos:
% \begin{itemize}
% \item[a)] Identifique uma região do plano que certamente contenha todas as raízes do problema. (dica: Elipse)
% \item[b)] Defina do Scilab a função $z=f(x,y)=\frac{x^2}{8}+\frac{(y-1)^2}{5}-1$ e $z=g(x,y)=\tan^{-1}(x)+x-y-y^3$
% \item[c)] Use os comandos \begin{verbatim}contour([-3:.1:3],[-2:.1:4],g,[0 0]) e contour([-3:.1:3],[-2:.1:4],f,[0 0])\end{verbatim}
% para traçar as curvas. Leia a ajuda do Scilab sobre esses comandos para entender melhor o que fazem.
% \item[d)] Com base no gráfico, encontre soluções aproximadas para o problema e use-as para inicias o método de Newton-Raphson. Encontre as raízes com erro inferior a $10^{-5}$.
% \end{itemize}
% 
% \begin{resp}
...
% \end{resp}
% 
% \begin{exer} (prova) Considere o sistema de equações dado por
% \begin{eqnarray*}
...
% 
% 
% \begin{exer}{\it (Otimização)} Uma indústria consome energia elétrica de três usinas fornecedoras. O custo de fornecimento em reais por hora como função da potência consumida em $kW$ é dada pelas seguintes funções
% \begin{eqnarray*}
...
% \end{eqnarray*}
% Calcule a distribuição de consumo que produz custo mínimo quando a potência consumida é  $1500kW$. Dica: Use a técnica dos multiplicadores de Lagrange, levando em conta que se $x_1$, $x_2$ e $x_3$ representam a potência consumida das três usinas, o custo total será dado por $C(x_1,x_2,x_3)=C_1(x_1)+C_2(x_2)+C_3(x_3)$ enquanto o consumo total é $x_1+x_2+x_3=1500$.
% \end{exer}
% 
% \begin{resp}
...
% \end{eqnarray*}
% 
% Resposta final: $(x_1,x_2,x_3)\approx (453.62,~ 901.94,~ 144.43)$
% \end{resp}
% 
% \begin{exer} \label{prob_ajuste_eax} Encontre a função do tipo $f(x)=Ab^{ x}$ que melhor aproxima os pontos $(0,3.1)$, $(1,4.4)$ e $(2,6.7)$ pelo critério dos mínimos quadrados. Dica: Você deve encontrar os valores de $A$ e $b$ que minimizam o erro dado por
% $$E_q=\left[3.1-f(0)\right]^2+\left[4.4-f(1)\right]^2+\left[6.7-f(2)\right]^2.$$
% {\bf Dica:} Para construir aproximações para resposta, considere a função $f(x)=Ab^x$ que passa pelo primeiro e terceiro ponto.
% \end{exer}
% \begin{resp}
...
% seguindo os seguintes passos:
% \begin{itemize}
% \item[a)] Defina do Scilab a função $z=f(x,y)=-x^4-y^6+3xy^3-x$
% \item[b)] Use o comando \begin{verbatim}contour([-2:.01:0],[-2:.01:0],f,[ 0:.2: 3])\end{verbatim}
% para traçar as curvas de nivel da função.
% \item[c)] Com base no gráfico, encontre valores aproximados para as coordenadas $xy$ do ponto de máximo.
% \item[d)] Sabendo que o ponto de máximo acontece quando o gradiente é nulo, escreva o problema como um sistema de duas equações não lineares e duas incógnitas.
% \item[e)] Implemente o método de Newton no Scilab.
% \end{itemize}
% Obs.: O comando \begin{verbatim}fplot3d([-2:.1:0],[-2:.1:0],f)\end{verbatim} pode ser usado para traçar um gráfico tridimensional da função $z=f(x,y)$, no entanto, em geral é mais prático analisar as curvas de nível. Explore este comando e use a ferramenta de rotação para visualizar melhor a função.
% \end{exer}
% \begin{resp}
...
% 
% 
% \begin{exer}A função $f(x,y,z)=\sin(x)+\sin(2y)+\sin(3z)$ possui um máximo quando $x=\pi/2$, $y=\pi/4$ e $z=\pi/6$. Use o procedimento descrito na questão anterior para calcular numericamente este ponto.
% \end{exer}
% 
...
% 
% \begin{resp}
% $x\approx 0.2982646, y\approx -0.2990796, z\approx- 1.6620333$  e $x\approx -0.0691328, y\approx 0.2923039, z\approx -.8235705$.
% \end{resp}
% 
% 
...
% \item[c)] Implemente o método de Newton-Raphson apartir das condições iniciais dadas pelos valores numéricos aproximados de $A=10$, $B=20$, $\lambda_1=\ln(2)$ e $\lambda_2=\ln(3)$ (ou seja, a solução exata acrescida de erros de arredondamento). Discuta.
% \end{itemize}
% \end{exer}
% 
% \begin{resp}
