...
\begin{tabular}{c|ccc}
  ...
  $c_3$  & $a_{31}$ & $a_{22}$ &      \\  \hline
         & $b_1$    & $b_2$    & $b_3$.
\end{tabular}
Na tabela mais à direita, omitimos os termo obrigatórimente nulos.
\end{center}
...
\subsection{Métodos de Runge-Kutta com dois estágios}
...
\end{eqnarray}
Este sistema é formada por três equações e quatro incógnitas, pelo que admite infinitas soluções. Para construir toda a família de soluções, escolha $\alpha\in (0,1] $ e defina a partir de \eqref{pvi:cond_euler_2est}:
$$b_1=1-\alpha,~~b_2=\alpha,~~c_2=\frac{1}{2\alpha},~~a_{21}=\frac{1}{2\alpha} $$
...
\begin{tabular}{c|cc}
   &   &   \\
  $\frac{1}{2\alpha}$ & $\frac{1}{2\alpha}$ &   \\  \hline
      & $1-\alpha$ &$\alpha$
\end{tabular},~~~$0<\alpha\leq 1$.
\end{center}
 Algumas escolhas comuns são $\alpha=1$, $\alpha=\frac{3}{4}$ e $\alpha=\frac{1}{2}$:
\begin{center}
\begin{tabular}{c|cc}
  $0$ &     &   \\
...
\end{tabular},
\end{center}
onde a última tabela fornece o método Euler modificado $\left(\alpha=\frac{1}{2}\right)$. O esquema iterativa assume a seguinte forma:
\begin{eqnarray}
  \tilde{u}_1 &=&u^{(n)} \\
  \tilde{u}_2 &=&u^{(n)}  +  \frac{h}{2\alpha}k_1 \\
  u^{(n+1)}&=&u^{(n)}  + h \left[ (1-\alpha)k_1+\alpha k_2\right],
\end{eqnarray}
onde $k_1=f(t^{(n)},u^{(n)})$ e $k_2=f(t^{(n)}+\frac{h}{2\alpha},\tilde{u}_2)$. Ou,equivalentemente:
\begin{eqnarray}
k_1&=&f(t^{(n)},u^{(n)})\\
k_2&=&f(t^{(n)}+\frac{h}{2\alpha},u^{(n)}  +  \frac{h}{2\alpha}k_1)\\
  u^{(n+1)}&=&u^{(n)}  + h \left[ (1-\alpha)k_1+\alpha k_2\right],
\end{eqnarray}


\subsection{Métodos de Runge-Kutta com três estágios}
...
consistem em iterações do tipo:
$$u^{(k+1)}=u^{(k)}+w_1 k_1 + \ldots + w_n k_n$$
onde
\begin{eqnarray*}
k_1&=&hf(u^{(k)},t_k),\\
k_2&=&hf(u^{(k)}+\alpha_{2,1}k_1,t_k+\beta_{2}h),\\
k_3&=&hf(u^{(k)}+\alpha_{3,1}k_1+\alpha_{3,2}k_2,t_k+\beta_{3}h),\\
&\vdots&\\
k_n&=&hf(u^{(k)}+\alpha_{n,1}k_1+\alpha_{n,2}k_2+\ldots \alpha_{n,n-1}k_{n-1},t_k+\beta_{n}h).\\
\end{eqnarray*}
aqui $n$ é o número de estágio do método.

Os coeficientes são escolhidos de forma a prover a ordem de precisão desejada.
...
No método de Euler melhorado, os coeficientes $\alpha$ e $\beta$ são dados por:
