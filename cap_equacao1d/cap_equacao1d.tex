...
Mostre que o teorema do ponto fixo se aplica a função $g(x) = \cos(x)$ no intervalo $[1/2, 1]$, i.e. que a iteração do ponto fixo converge para a solução da equação $\cos x = x$. Então, calcule as iterações do ponto fixo com aproximação inicial $x^{(1)} = 0,7$ e estime o erro absoluto da aproximação e sua variação a cada iteração.
\end{ex}
\begin{sol}
...
\end{eqnarray*}
Para estimar o erro, consideramos $x^* = 0,7390851605$.

\begin{table}
  \centering
  \begin{tabular}{l|ccc}\hline
   $n$ & $x^{(n)}$ & $\epsilon_n := |x^* - x^{(n)}|$ & $|\epsilon_n-\epsilon_{n-1}|$\\\hline
   $1$ & $0,70000$ & $8,7\E-01$ & -- \\
   $2$ & $0,76484$ & $2,6\E-02$ & $1,3\E-02$\\
   $3$ & $0,72149$ & $1,8\E-02$ & $8,2\E-03$ \\
   $4$ & $0,75082$ & $1,2\E-02$ & $5,9\E-03$ \\
   $5$ & $0,73113$ & $8,0\E-03$ & $3,8\E-03$ \\
   $6$ & $0,74442$ & $5,3\E-03$ & $2,6\E-03$ \\
   $7$ & $0,73548$ & $3,6\E-03$ & $1,7\E-03$ \\\hline
  \end{tabular}
...
%%%%%%%%%%%%%%%%%%%%
\ifisscilab
No \verb+Scilab+, podemos computar estas iterações, o erro absoluto e sua variação com o seguinte código:
\begin{verbatim}
//est. da solucao
...
%%%%%%%%%%%%%%%%%%%%
\ifisoctave
No \verb+GNU Octave+, podemos computar estas iterações, o erro absoluto e sua variação com o seguinte código:
\begin{verbatim}
#est. da solucao
f = inline("cos(x)-x");
xe = fsolve(f, 0.7);
...
%%%%%%%%%%%%%%%%%%%%
\ifispython
Em \verb+Python+, podemos computar estas iterações, o erro absoluto e sua variação com o seguinte código:
\begin{verbatim}
#funcao do pto. fixo
def g(x):
    return np.cos(x)
...
for i in np.arange(2,8):
  x = g(x0);
  eps = np.fabs(x-xe);
  print("%1.5f %1.1e %1.1e\n" % (x, eps, np.fabs(eps-eps0)))
  x0 = x
  eps0 = eps
\end{verbatim}
\fi
