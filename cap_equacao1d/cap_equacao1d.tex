...
Aplicando esse método repetidas vezes, podemos construir a iteração
\begin{eqnarray*}
x^{(1)}&=&r \\
...
\begin{ex}
Para obter uma aproximação para $\sqrt{5}$, podemos iniciar com a aproximação inicial $r=2$ e $A=5$.
...
obtemos
\begin{align*}
x^{(2)}=&\frac{2}{2}+\frac{2,5}{2}=1+1,25=2,25\\
...
A experimentação numérica sugere que o método funciona, mas surgem três perguntas:
\begin{enumerate}
...
A partir dessas duas equações podemos propor dois processos iterativos diferentes:
\begin{enumerate}
 \item [a)] $\displaystyle x^{(n+1)}=\ln \left(\frac{10}{x^{(n)}}\right)$
 \item [b)] $\displaystyle x^{(n+1)}=10 e^{-x^{(n)}} $
\end{enumerate}

Supondo que $x^{(1)}=1$ é uma aproximação inicial para a solução deste problema, iterando a primeira equação temos
\begin{align*}
x^{(1)} &=1\\
x^{(2)} &=\ln\left({10}\right)=2,3025851\\
x^{(3)} &=\ln\left(\frac{10}{2,3025851}\right)=1,4685526\\
x^{(4)} &=\ln\left(\frac{10}{1,4685526}\right)=1,9183078 \\
x^{(5)} &=\ln\left(\frac{10}{1,9183078}\right)=1,6511417  \\
        &\vdots\\
x^{(11)}&=1,7421335 \\
x^{(21)}&= 1,7455151\\
x^{(31)}&= 1,745528\\
...
O experimento numérico sugere que a iteração $a$ converge para $1,745528...$ e a iteração $b$ não é convergente.
...
\begin{defn}
 ...
 $$|g(x)-g(y)|\leq \beta |x-y|,~~~0\leq \beta<1.$$
\end{defn}

\begin{obs}
A desigualdade $|g(x)-g(y)|\leq \beta |x-y|$ implica que $g$ é contínua.

Assim, $g$ é uma contração em $[a,b]$  se $|g'(x)|<1$ para $a<x<b$.
\end{obs}

\begin{teo}
 Se $g:[a,b]\to [a,b]$ é uma contração, então existe um único ponto fixo $p\in [a,b]$ tal que $g(p)=p$.

 Além disso, a sequência
  $$x^{(n+1)}=g(x^{(n)})$$
é convergente para qualquer $x^{(1)}\in[a,b]$ e $$\lim_{n\to\infty}x^{(n)}=p.$$
\end{teo}
...
e
$$f(b)=b-g(b)\geq b-b=0$$
Se $f(a)=a$ ou $f(b)=b$, então o ponto fixo existe. Caso contrário, as desigualdade são estritas e a função muda de sinal no intervalo.  Como a função é contínua, pelo teorema de Bolzano~\ref{teo:teorema_de_Bolzano}, existe um ponto $x^*$ no intervalo $(a, b)$ tal que $f(x^*)=0$, ou seja, $x^*-g(x^*)=0$. Observe que $x^*$ é um ponto fixo de $g$, pois $g(x^*)=x^*$. 
...
Observações:
\begin{itemize}
\item A condição $|g(x)-g(y)|\leq \beta |x-y|$ é satisfeita sempre que $|g'(x)|\leq \beta<1$ em todo o intervalo pois
\begin{align*}
  |g(x)-g(y)| &=\left|\int_x^y g'(s)\;ds\right|\leq \int_x^y|g'(s)| \;ds \\
              &\leq \int_x^y\beta \;ds =\beta|x-y|,~~ x<y. 
\end{align*}
...
\begin{teo}
 Se $g\in C^1[a,b]$ e  $|g'(x^*)|<1$, então $x^*$ é estável. Se $|g'(x^*)|>1$ é instável e o teste é inconclusivo se $|g'(x^*)|=1$.
\end{teo}
...
&=&(x^{(N+1)}-x^{(N)}) \frac{1}{1-g'(x^*)}, \left|g'(x^*)\right|<1
\end{eqnarray*}
...
{\bf Conclusões:} Tendo em mente a relação $x^{(n+1)}-x^{(n)}  \approx (x^{(n)}-x^{(n-1)}) g'(x^*)$, concluímos:
\begin{itemize}
\item Quando $g'(x^*)<0$, o esquema é alternante e o erro $\epsilon_N$ pode ser estimado diretamente da diferença $\Delta_N$.
\item Quando $g'(x^*)>0$, o esquema é monótono e $\frac{1}{1-g'(x^*)}>1$, pelo que o erro $\epsilon_N$ é maior que a diferença $\Delta_N$. A relação será tão mais importante quando mais próximo da unidade for $g'(x^*)$, ou seja, quando mais lenta for a convergência.
\item Como $g'(x^*)\approx \frac{x^{(n+1)}-x^{(n)}}{x^{(n)}-x^{(n-1)}}$, temos
...
Consideremos o problema de encontrar as raízes da equação
...
Queremos obter $x^{(1)}$ a partir de $x^{(0)}$ sabendo que $x^{(1)}$ é a intersecção da reta tangente em $x^{(0)}$ com o eixo $x$. A inclinação da reta tangente é dada por (a variação em $y$ sobre a variação em $x$)
\begin{align*}
   f'(x^{(0)}) = \frac{ f(x^{(0)})-0}{x^{(0)}-x^{(1)} }
\end{align*}
onde isolando $x^{(1)}$ obtemos
\begin{align*}
   x^{(0)}-x^{(1)} &= \frac{ f(x^{(0)})-0}{f'(x^{(0)})}
\end{align*}
Portanto
