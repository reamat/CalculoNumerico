...
\begin{eqnarray}
  u'(t) &=& f(t, u(t)),~~t>t^{(1)}\label{problema de valor inicial_EDO}\\
  u(t^{(1)}) &=& a ~~ \text{(condição inicial)}.\label{problema de valor inicial_CI}
\end{eqnarray}
\end{subequations}
...
u^{(1)}&=&u_1=u(t_1)~ \hbox{(condição inicial)}.
\end{eqnarray}
...
Itere a fórmula
\begin{eqnarray}
  u_{n+1}=u_n + h( -0,5u_n+2+t_n), \quad  u^{(1)}=8
\end{eqnarray}
